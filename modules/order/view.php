...
if(isset($_POST['submit'])){
   ...
   $orders = "SELECT * FROM tbl_orders ord INNER JOIN tbl_employee sales ON ord.employee_id = sales.employee_id WHERE sales.added_by = '$login_id' AND ord.created_at BETWEEN '$start_date' AND '$end_date' ";
   
}else if(isset($_GET['id']) && $_GET['id'] != ""){

   $orders = "SELECT * FROM tbl_orders ord INNER JOIN tbl_employee sales ON ord.employee_id = sales.employee_id WHERE ord.meeting_id = '".$_GET['id']."' ";

}else{
   
   $orders = "SELECT * FROM tbl_orders ord INNER JOIN tbl_employee sales ON ord.employee_id = sales.employee_id WHERE sales.added_by = '$login_id'";
                                          ...
                                          <tr>
                                             ...
                                             <td class="text-center">
                                                <?php 
                                                   ...
                                                   $assign_godowns = getWhere('tbl_order_detail','order_id',$rs['order_id']);


                                                      $assign_status = 0;

                                                      foreach($assign_godowns as $val){
                                                      ...
                                                      if($assign_status == 1){
                                                         ...
                                                         echo "<a href='assign.php?order_id=".$rs['order_id']."'>assign</a>";                                      
                                                         
                                                      }else{

                                                         $max_count = count($assign_godowns);

                                                         $k=1;
                                                         foreach($assign_godowns as $val){

                                                            $godown_name = getOne('tbl_godown','godown_id',$val['godown_id']);

                                                            if($k == $max_count){
                                                               echo "<span class='label label-primary'>".$godown_name['godown_name']."</span>";
                                                            }else{
                                                               echo "<span class='label label-primary'>".$godown_name['godown_name']."</span> ";
                                                            }
                                             ...
                                             <td class="text-center"> 
                                                <?php 
                                                   $dispatch_status = "SELECT SUM(order_product_quantity) as required_qty,SUM(order_dispatch_quantity) as dispatch_qty FROM tbl_order_detail WHERE order_id = '".$rs['order_id']."' ";
